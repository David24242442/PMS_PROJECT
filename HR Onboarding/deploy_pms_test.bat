@echo off
setlocal

set "SHARE_PATH=\\192.168.0.24\it-software\IT DEV DAVID\PMS"
set "FRONTEND_SRC=c:\Users\USER\Workspaces\htdocs\HR Onboarding\hrproject Vue\hrproject"
set "BACKEND_SRC=c:\Users\USER\Workspaces\htdocs\HR Onboarding\hr\hr"

set "FRONTEND_DEST=%SHARE_PATH%\PMS_FRONTEND_VUE"
set "BACKEND_DEST=%SHARE_PATH%\pms_backend_laravel"

echo ===========================================
echo   PMS Project Replacement Deployment 
echo ===========================================
echo.

:: 1. Build Frontend
echo [1/3] Building Vue Frontend (Production)...
cd /d "%FRONTEND_SRC%"
call npm run build
if %errorlevel% neq 0 (
    echo [ERROR] Frontend build failed!
    pause
    exit /b %errorlevel%
)

:: 2. Clean and Transfer Frontend
echo [2/3] Cleaning and Mirroring Frontend to Share...
:: Using robocopy /MIR will effectively "delete and replace" to match source exactly
robocopy "%FRONTEND_SRC%\dist" "%FRONTEND_DEST%" /MIR /MT /R:2 /W:5 /NP
if %errorlevel% geq 8 echo [ERROR] Frontend transfer failed with error %errorlevel%

:: 3. Clean and Transfer Backend
echo [3/3] Cleaning and Mirroring Backend to Share...
:: Exclude vendor and node_modules as they are huge and should stay on server or be installed there
echo Transferring Laravel files (Mirroring source, excluding vendor/nodes)...
robocopy "%BACKEND_SRC%" "%BACKEND_DEST%" /MIR /MT /R:2 /W:5 /NP /XD vendor node_modules .git storage/logs storage/framework/sessions storage/framework/views /XF .env.local .env.development

echo.
echo ===========================================
echo   SUCCESS! PMS project replaced by HR Onboarding.
echo   Files are ready at: %SHARE_PATH%
echo ===========================================
pause
